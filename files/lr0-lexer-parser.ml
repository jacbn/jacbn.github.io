...
let productionToSymbolList = function | Production(_, ps) -> ps | EpsilonProduction _ -> [T Epsilon];; (*should this be []??*)
...
replaceRightmost (Br(Start, [Br(Aterm, []); Br(Sterm', [])])) [Lf Epsilon];;